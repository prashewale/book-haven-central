...
export function TabbedProducts() {
  const featured = useMemo(() => BOOKS.filter((b) => b.isBestseller || b.onSale).slice(0, 8), []);
  const newArrivals = useMemo(() => BOOKS.filter((b) => b.isNewRelease).slice(0, 8), []);
    ...
    <section className="py-20">
      <div className="container mx-auto px-4">
        <div className="text-center mb-10 space-y-2">
          <h2 className="text-3xl md:text-4xl font-serif font-bold">Our Collection</h2>
          <p className="text-muted-foreground">Curated picks for every reader</p>
        </div>
