...
export function OnSaleCountdown() {
  const saleBooks = BOOKS.filter((b) => b.onSale).slice(0, 8);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-4">
          ...
          <div className="flex gap-2">
            {[
              ...
              <div key={unit.label} className="flex flex-col items-center min-w-[52px] p-2.5 bg-background rounded-lg shadow-soft border">
                <span className="text-xl font-bold tabular-nums">{unit.val.toString().padStart(2, '0')}</span>
                <span className="text-[10px] uppercase tracking-widest text-muted-foreground">{unit.label}</span>
              </div>
            ))}
          </div>
        </div>
        <BookSlider books={saleBooks} minVisible={6} autoSlideInterval={4000} />
      </div>
    </section>
  );
}